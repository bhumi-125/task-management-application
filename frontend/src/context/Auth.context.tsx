...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
    ...
    const loginUser = async (email: string, password: string) => {
        ...
        await profileUser(data.token); // Set user after login
        return data;
    };
    ...
    const profileUser = async (token: string) => {
        ...
        if (data.statusCode === 400) {
            ...
            setUser(data.user); // Set user data after login
        }
    };
    ...
    return (
        <AuthContext.Provider value={{ registerUser, loginUser, profileUser, user, setUser, logoutUser }}>
            {children}
